...
__FORMSPACE__INTERNAL__CalculateAdjoint := function(mat, mode, hom, n)
...
local k, i, W, first, j, Cond, Conds, h_star, h, w, O, needs_checking, A, failed_check, nspace, vec;
    ...
    for i in [1..d] do
        ...
        h := Gens[i];
        h_star := __FORMSPACE__INTERNAL__CalculateAdjoint(h, unitary, hom, n);
        for j in [1..n] do
    ...
    for w in W do
        ...
        if needs_checking then
            failed_check := false;
            for i in [1..d] do
                if not failed_check and Gens[i] * A * __FORMSPACE__INTERNAL__CalculateAdjoint(Gens[i], unitary, hom, n) <> Lambdas[i] * A then
                    failed_check := true;
                fi;
            od;
