...
# This function returns [i0,a] such that if <mat> preserves a bilinear modulo scalars
# then the scalar lambda satisfies lambda^i0 = a
##
...
InstallGlobalFunction( ClassicalForms_GeneratorsWithoutScalarsDual,
  function( grp )
    local tries, gens, field, m1, a1, new, i, scalars;
    ...
    for i in [1..Length(gens)] do 
        ...
        if IsList(a1) then 
            if a1[1] = 1 then # the matrix m1 has scalar a1[2]
                scalars[i] := [a1[2]];
            elif IsEvenInt(a1[1]) then # modify m1 to a matrix that has scalar one.
                scalars[i] := [One(field)];
                gens[i] := m1/NthRoot(field,a1[2],2*a1[1]);
            else
                scalars[i] := AsList(Group(NthRoot(field,a1[2],a1[1]))); # add all possible scalars for m1 
            fi;
        fi;
    od;
    
    # new together with scalars gives enough information to construct all duals.
    new := GModuleByMats( gens, field );
    return [new,scalars];
    
    #if MTX.IsAbsolutelyIrreducible(new) then
    #    return new;
    #fi;
    
    # This group generated by gens fixes the form up to scalar multiples and is not absolutely irreducible 
